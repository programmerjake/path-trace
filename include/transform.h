...
public:
    ...
    static Matrix rotate(const Vector3D axis, const double angle)
    {
        Vector3D axisv = normalize(axis);
        float c, s, v;
        c = cos(angle);
        s = sin(angle);
        v = 1 - c; // Versine
     ...
     * the same as <code>Matrix::rotate(Vector3D(1, 0, 0), angle)</code>
     *
     * @param angle
     *            angle to rotate around the x axis in radians
     * @return the new rotation matrix
     * @see #rotate(Vector3D axis, double angle)
     * @see #rotateY(double angle)
     * @see #rotateZ(double angle) */
    ...
    /** creates a rotation matrix<br/>
     * the same as <code>Matrix::rotate(Vector3D(0, 1, 0), angle)</code>
     *
     * @param angle
    ...
    /** creates a rotation matrix<br/>
     * the same as <code>Matrix::rotate(Vector3D(0, 0, 1), angle)</code>
     *
     * @param angle
    ...
    static Matrix translate(Vector3D position)
    {
        return Matrix(1,
                          0,
                          0,
     ...
     * @return the new scaling matrix */
    static Matrix scale(Vector3D s)
    {
    ...
    {
        return Vector3D(v.x * this->x00 + v.y * this->x10 + v.z * this->x20
                + this->x30, v.x * this->x01 + v.y * this->x11 + v.z * this->x21
                + this->x31, v.x * this->x02 + v.y * this->x12 + v.z * this->x22
                + this->x32);
    }

    Vector3D applyNoTranslate(Vector3D v) const
    {
        return Vector3D(v.x * this->x00 + v.y * this->x10 + v.z * this->x20, v.x
                * this->x01 + v.y * this->x11 + v.z * this->x21, v.x * this->x02
                + v.y * this->x12 + v.z * this->x22);
    }
};

inline Vector3D transform(const Matrix & m, Vector3D v)
{
    return m.apply(v);
}

inline Ray transform(const Matrix & m, Ray v)
{
    return Ray(m.apply(v.origin), m.applyNoTranslate(v.dir));
}
